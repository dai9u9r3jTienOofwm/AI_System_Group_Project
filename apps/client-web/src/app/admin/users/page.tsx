...
export default function UsersPage() {
  ...
  const handleAddUser = () => { setEditingUser(null); form.resetFields(); setIsModalVisible(true); };
  const handleEditUser = (user: User) => { setEditingUser(user); form.setFieldsValue(user); setIsModalVisible(true); };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: unknown, record: User) => (
        <Space size="small">
          <Button type="text" icon={<EditOutlined />} onClick={() => handleEditUser(record)} size="small">Sửa</Button>
          <Popconfirm
            title="Xác nhận xóa"
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(15,23,42,0.9),_rgba(2,6,23,1)_55%,_rgba(0,0,0,1)_100%)] px-6 py-10 text-slate-100 md:px-10">
      ...
      <Card className="border border-white/10 bg-slate-950/70 text-slate-100 shadow-[0_0_40px_rgba(0,0,0,0.45)]">
        <Button type="primary" icon={<PlusOutlined />} onClick={handleAddUser} className="mb-4">
          Thêm Người Dùng
        </Button>
        <Table
          columns={columns}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="role" label="Vai Trò" rules={[{ required: true, message: 'Vui lòng chọn vai trò' }]}>
            <Select options={[{ label: 'Admin', value: 'admin' }, { label: 'Editor', value: 'editor' }, { label: 'Viewer', value: 'viewer' }]} />
          </Form.Item>
          {!editingUser && (
